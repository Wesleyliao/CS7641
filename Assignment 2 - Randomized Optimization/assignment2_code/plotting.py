    ...
    for pop in ga_files.keys():
        ...
        for i, mate in enumerate(ga_files[pop].keys()):
            ...
            if nn_curve:
                ...
                main_df[mate] = reduce(lambda x, y: pd.merge(x, y, left_index = True, right_index = True), main_df[mate])
...
def plot_sa_data(problem_name, sa_files, output_dir, nn_curve=False):
    ...
    if nn_curve:
        ...
        main_df = reduce(lambda x, y: pd.merge(x, y, left_index = True, right_index = True), main_df)
