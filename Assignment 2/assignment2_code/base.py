...
def f1_score(labels_in, predicted_in):
    ...
    for j in range(NUM_OF_CLASSES):
        ...
        labels = [1. if x == j else 0. for x in labels_in]
        predicted = [1. if x == j else 0. for x in predicted_in]
        ...
        # print('label {} tp {}'.format(j, tp))
        # print('label {} tn {}'.format(j, tn))
        # print('label {} fp {}'.format(j, fp))
        # print('label {} fn {}'.format(j, fn))
        if tp == 0:
...
def error_on_data_set(network, ds, measure, ugh=False):
    ...
    for instance in ds:
        ...
        actual = convert_to_labels(instance.getLabel().getData())

        predicted = convert_to_labels(network.getOutputValues())

        if ugh:
            # print("actual: ", instance.getLabel().getData())
            # print("predicted: ", network.getOutputValues())
