...
class FashionData(DataLoader):

    def __init__(self, path='data/fashion-mnist_test.csv', verbose=False, seed=1):
...
class NBAData(DataLoader):

    def __init__(self, path='data/NBAshotdata.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def _preprocess_data(self):
        temp = self._data['SHOT_RESULT']
        self._data = self._data.drop(['SHOT_RESULT', 'CLOSEST_DEFENDER_PLAYER_ID', 'FGM', 'PTS', 'player_id', 'GAME_ID', 'MATCHUP', 'W', 'FINAL_MARGIN'], 1)
        self._data['SHOT_CLOCK'] = self._data['SHOT_CLOCK'].fillna(self._data['SHOT_CLOCK'].mean()).astype(float)
        self._data['GAME_CLOCK'] = pd.to_timedelta('0:' + self._data['GAME_CLOCK']).dt.total_seconds()
        self._data['CLOSEST_DEFENDER'] = self._data['CLOSEST_DEFENDER'].str.replace(', ','_')
        self._data['player_name'] = self._data['player_name'].str.replace(' ','_')
        self._data['PTS_TYPE'] = self._data['PTS_TYPE'].astype(str)
        self._data = pd.get_dummies(self._data, prefix = ['LOCATION', 'PTS_TYPE', 'CLOSEST_DEFENDER', 'player_name'])
        self._data['SHOT_RESULT'] = temp
        self._data = self._data.astype(float)
        self._data.to_csv("NBA_final_feed.csv")
    def pre_training_adjustment(self, train_features, train_classes):
        """
        Perform any adjustments to training data before training begins.
        :param train_features: The training features to adjust
